...
function isMatchLocked(m) {
  ...
  const dt = new Date(m['Дата'] + 'T' + (m['Время'] || '00:00') + ':00');
...
function renderHome() {
  ...
  const upcoming = state.matches.filter(m => m['Статус'] !== 'Завершён')
    .sort((a,b) => new Date(a['Дата']+' '+a['Время']) - new Date(b['Дата']+' '+b['Время'])).slice(0,5);
...
function renderMatchCard(m, showButton) {
  ...
  return `<div class="match-card">
    <div class="match-meta"><span>${formatDate(m['Дата'])} · ${m['Время']}</span><span class="status-badge">${m['Статус']||'Не начался'}</span></div>
...
function openProfile(username) {
  ...
  const exactCount = scored.filter(p => Number(p['Баллы']) >= 3).length;
  ...
  const active = enriched.filter(x => !isMatchLocked(x.match)).sort((a,b) => new Date(a.match['Дата']+' '+a.match['Время']) - new Date(b.match['Дата']+' '+b.match['Время']));
...
async function submitPrediction() {
  ...
  if (isMatchLocked(state.currentMatch)) { showToast('Матч уже начался, прогноз не принят'); closeModal(); loadData(); return; }
  localStorage.setItem('wc_username', username);
  const payload = {
    matchId: state.currentMatch['ID'], username, outcome: state.selections.outcome,
    scoreHome, scoreAway, btts: state.selections.btts, total: state.selections.total, confidence: state.selections.confidence
  };
  const ok = await sendPrediction(payload);
  if (ok) { showToast('Прогноз сохранён!'); closeModal(); setTimeout(loadData, 800); }
  else { showToast('Ошибка отправки, попробуй снова'); }
}

document.querySelectorAll('.tab').forEach(tab => {
  tab.addEventListener('click', () => {
    if (tab.dataset.page === 'profile') { openMyProfile(); return; }
    document.querySelectorAll('.tab').forEach(t => t.classList.remove('active'));
    document.querySelectorAll('.page').forEach(p => p.classList.remove('active'));
